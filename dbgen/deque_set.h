...
// Merge two sorted deques, freeing memory as we go (so that both inputs are modified).
// The first argument, m_a, will contain the set union, while the second input, m_b,
// will be emptied.
template <class T> 
void unique_merge(std::deque<T> &m_a, std::deque<T> &m_b)
{
	std::deque<T> dst;
	
	while( !m_a.empty() && !m_b.empty() ){
		
		if( m_a.front() < m_b.front() ){
			
			dst.push_back( m_a.front() );
			m_a.pop_front();
		}
		else{
			if( m_a.front() > m_b.front() ){
			
				dst.push_back( m_b.front() );
				m_b.pop_front();
			}
			else{ // m_a == m_b
